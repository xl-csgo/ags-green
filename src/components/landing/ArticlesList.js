...
const ArticlesList = () => {
    ...
    return ( 
      ...
      {articles.map((article) => (
        <div className="a" key={article.documentId}>
          <img
            src={`http://localhost:1337${article.thumbnail.formats.thumbnail.url}`}
            alt={article.title || 'Article thumbnail'}
          />
          {article.redirect === "none" || article.redirect === null
            ? <Link to={`/article/${article.documentId}`}><h2>{article.title}</h2></Link>
            : <Link to={article.redirect}><h2>{article.title}</h2></Link>}
